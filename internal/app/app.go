...
func Run(ctx context.Context, cfg *config.Config, log logger.Logger) error {
	...
	teleRepo := repository.NewTelegramRepository(db)

	if teleSender != nil {
		eg.Go(func() error {
			log.Info("starting telegram polling")
			teleSender.StartPolling(ctx, func(ctx context.Context, username string, chatID int64) error {
				return teleRepo.SaveSubscriber(ctx, username, chatID)
			})
			return nil
		})
	}
	...
	svc, err := initNotifyService(&cfg.Service, db, tm, rdb, multiSender, publisher, teleRepo, log)
	if err != nil {
		return fmt.Errorf("init service: %w", err)
	}
	handler := httpt.NewNotifyHandler(svc, log)
	startWorkers(ctx, eg, svc, rmq, handler, cfg, log)
...
func startQueueProcessor(
	...
	for {
		...
		case <-ctx.Done():
			return fmt.Errorf("queue processor stopped: %w", ctx.Err())
		}
	}
}
...
func initNotifyService(
	...
	teleRepo *repository.TelegramRepository,
	log logger.Logger,
...
func startWorkers(
	...
	eg.Go(func() error {
		return startQueueProcessor(ctx, log, svc, cfg.Publisher.QueueProcessorInterval)
	})

	for _, ch := range entity.ListChannels() {
		eg.Go(func() error {
			return runConsumer(ctx, svc, rmq, log, ch.String(),
				cfg.Publisher.RabbitMQWorkers,
				cfg.Publisher.RabbitMQPrefetchCount,
			)
		})
	}
}
